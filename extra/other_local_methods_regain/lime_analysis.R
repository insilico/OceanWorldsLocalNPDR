...
###### lime on NPDR selected features on train data, which rf already seen
...
explain_train <- lime::explain(testdat, lime_explainer,
                    n_labels = 1, 
...
long_data <- read.csv("lime_output.csv", header = TRUE)
colnames(long_data) 
...
make_lime_matrix <- function(lime_df){
  ...
  unique_features = data.frame(features=unique(long_data$feature)) %>% filter(features!="class")
  case_id_vec <- unique(lime_df$case)
  ...
  for (curr_case in case_id_vec){
    ...
    feaure_names <- curr_case_df$feature
    score_mat[curr_case,] <- curr_case_df$feature_weight
  }
  colnames(score_mat) <- unique_features 
  return(score_mat)
}
make_lime_matrix(long_data)
unique_features = data.frame(features=unique(long_data$feature)) %>% filter(features!="class")
unique_features$features
case_id_vec <- unique(long_data$case)
score_mat <- matrix(0, nrow = length(case_id_vec), 
                    ncol=length(unique_features$features))
for (curr_case in case_id_vec){
  cat(curr_case, "\n")
  long_data %>% filter(case==curr_case) %>%
    select(feature,feature_weight) -> curr_case_df
  feaure_names <- curr_case_df$feature
  score_mat[curr_case,] <- curr_case_df$feature_weight
  cat(curr_case_df$feature_weight, "\n")
}
colnames(score_mat) <- unique_features$features 
score_mat

#wide_data <- long_data %>% distinct() %>% #mutate(row = row_number()) %>%
#  select(feature, label, feature_value, feature_weight) %>%
#  pivot_wider(names_from = feature, 
#              values_from = c(feature_value, feature_weight))
